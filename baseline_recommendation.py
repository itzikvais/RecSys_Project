...
#    bestAccuracys.to_csv("NeedConcern/"+topic+"_Accuacy.csv",index_label=False)
    pd.DataFrame(pre).to_csv(os.path.join(OUTPUT_DIR_PATH,"NeedConcern/topic_"+topic+"_"+str(TrackNum)+"_recomend.csv"),index_label=False)
...
def discuss(max_evals):
    ...
    #歌曲，歌单，歌手，专辑关系
    tracksWithDetail=pd.read_csv(os.path.join(OUTPUT_DIR_PATH,"NeedConcern/trickWithDetail.csv"))[["album_uri","artist_uri","track_uri","pid"]]
    #待验证的歌单id
    ...
    for t in topicOfPlaylist:
        ...
        if(len(needTest)==0 ):#or TrackNum<500 or len(currentPid)>200 ):
            ...
            print(best['pw'],best['aw'],best['tw'],"Average accuracy rate under a single subject",MAP,mndcg)
    ...
    allAccuracy.to_csv(os.path.join(OUTPUT_DIR_PATH,"NeedConcern/allAccuracy_detail_withNDCG_node2vec.csv"),index_label=False)
    print("Done!")                
    return
discuss(30)
