...
corpus,n = gettxt(os.path.join(OUTPUT_DIR_PATH,"Output1/"),pidlist)

#corpus.extend(testcorpus)
cntVector = CountVectorizer(stop_words={'english'},encoding='utf-8')
cntTf = cntVector.fit_transform(corpus)
joblib.dump(cntVector,os.path.join(OUTPUT_DIR_PATH,"cntVector.model" ))
#joblib.dump(cntVector,"cntTf.model" )
print("cntV完成")
#LDA主题模型训练        K=20 -50
from sklearn.decomposition import LatentDirichletAllocation
n_topics = range(116,117)
...
joblib.dump(best_model,os.path.join(OUTPUT_DIR_PATH,"best_model_116_topics.model" ))
...
doc_topic.to_csv(os.path.join(OUTPUT_DIR_PATH,"doc_topic_116.csv"),index=False)
pd.DataFrame(best_model.components_).to_csv(os.path.join(OUTPUT_DIR_PATH,"topic_word1_116.csv"),index=False)
